...
namespace Assets.Scripts
{
    public class CueController : MonoBehaviour
    {
        ...
        {
            if (Input.GetAxis(InputAxes.HeightCue) != 0)
            {
                _moveVertical = Input.GetAxis(InputAxes.HeightCue);
            }
            ...
            if (Input.GetAxis(InputAxes.DirectionPadX) != 0)
            {
                _rotateVertical = Input.GetAxis(InputAxes.DirectionPadX);
            }

            if ((_power > 0 && _moveVertical > 0) || (_power < 1 && _moveVertical < 0))
            {
                _rigidbody.transform.Translate(new Vector3(0, 0, _moveVertical) * 0.01f);
            }
